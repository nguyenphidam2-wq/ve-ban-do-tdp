...
async function main() {
  ...
  console.log('--- CHI TIẾT CÁC LINESTRING & POLYGON ---');
  for (let i = 0; i < placemarks.length; i++) {
    ...
    if (lines.length > 0) {
      const coords = parseCoordinates(lines[0].getElementsByTagName('coordinates')[0].textContent);
      const first = coords[0];
      const last = coords[coords.length - 1];
      const isClosed = Math.abs(first[0] - last[0]) < 1e-6 && Math.abs(first[1] - last[1]) < 1e-6;
      console.log(`LineString: "${name}" | Số điểm: ${coords.length} | Khép kín: ${isClosed ? 'CÓ' : 'KHÔNG'}`);
    }
